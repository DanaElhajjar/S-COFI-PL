...
from script_simulation_COFIPL_vs_SCOFIPL_compar_SCM_PO import parallel_Monte_Carlo
...
if __name__ == "__main__":
    ...
    delta_phases_MM_seq_ones_PO = [[] for i in range(len(args.n_list))] 
    delta_phase_MM_offline_PO = [[] for i in range(len(args.n_list))]

    for key, value in enumerate(args.n_list):
        delta_phases_MM_seq_ones[key], delta_phases_MM_seq_ones_PO[key], delta_phase_MM_offline[key], delta_phase_MM_offline_PO[key] = parallel_Monte_Carlo(value, trueCov, args.parameters, args.n_trials, args.n_threads, args.Multi)
    ...
    MSE_delta_phases_MM_seq_ones_PO = calculateMSE(delta_phases_MM_seq_ones_PO, true_delta_new, args.n_trials, args.n_list)
    MSE_delta_phase_MM_offline_PO = calculateMSE(delta_phase_MM_offline_PO, true_delta, args.n_trials, args.n_list)

    for m in range(0, args.k):
        plt.figure()
        plt.xlabel('n')
        plt.ylabel('MSE')
        plt.plot(args.n_list, np.array(MSE_delta_phases_MM_seq_ones)[:,m],'v-', color ='purple', label = 'S-COFI-PL SCM')
        plt.plot(args.n_list, np.array(MSE_delta_phases_MM_seq_ones_PO)[:,m],'v-', color ='green', label = 'S-COFI-PL PO')
        plt.plot(args.n_list, np.array(MSE_delta_phase_MM_offline)[:,args.p+m],'v-', color ='blue', label = 'COFI-PL SCM')
        plt.plot(args.n_list, np.array(MSE_delta_phase_MM_offline_PO)[:,args.p+m],'v-', color ='pink', label = 'COFI-PL PO')
        plt.legend()
        plt.grid("True")
